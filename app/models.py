...
class Brand(db.Model):
	...
	def __init__(self, name, avg_price, avg_rating, num_products, image_url):
		...
		assert isinstance(avg_price, int) or avg_price == None
		assert isinstance(avg_rating, int) or avg_rating == None
		assert isinstance(num_products, int) or num_products == None
		assert isinstance(image_url, str) or image_url == None
...
class Product(db.Model):
	...
	def __init__(self, brand_id, name, description, price, rating, image_url)
		...
		assert isinstance(price, int) or price == None
		assert isinstance(rating, int) or rating == None
		assert isinstance(image_url, str) or image_url == None
...
class Color(db.Model):
	...
	def __init__(self, name, hashcode, num_products)
		...
		assert isinstance(num_products, int) or price == None
...
class ProductClassificationCategory(db.Model):
	...
	product_id = db.Column('product_id',db.Integer, db.ForeignKey('product.product_id'), primary_key=True)
	classification_id = db.Column('classification_id',db.Integer, db.ForeignKey('classification.classification_id'), primary_key=True)
	category_id = db.Column('category_id',db.Integer, db.ForeignKey('category.category_id'), primary_key=True, nullable=True)

	def __init__(self, product_id, classification_id, category_id):
		assert isinstance(product_id, int) 
		assert isinstance(category_id, int)
		assert isinstance(classification_id, int) or classification_id == None
