...
class UnitTests(TestCase):
	...
	def test_model_brand_1 (self):
		 with app.test_request_context():
		 	brand = Brand('Pure Anada', 1.01, 4.02, 2, 'http://entertainment.inquirer.net/files/2016/07/13717225_1265259390150735_8093269019210020606_o.jpg')
		 	
		 	db.session.add(brand)
		 	db.session.commit()

		 	query = db.session.query(Brand).filter_by(name='Pure Anada').first()
		 	self.assertEqual(query.avg_price, 1.01)
		 	self.assertEqual(query.avg_rating, 4.02)
		 	self.assertEqual(query.num_products, 2)
		 	self.assertEqual(query.image_url, 'http://entertainment.inquirer.net/files/2016/07/13717225_1265259390150735_8093269019210020606_o.jpg')
	...
	def test_model_product_brand_1 (self):
		 with app.test_request_context():
		 	brand = Brand('Pure Anada', 1.01, 4.02, 2, 'http://entertainment.inquirer.net/files/2016/07/13717225_1265259390150735_8093269019210020606_o.jpg')
		 	product = Product(1,'Makeup','This is makeup', 1.99, 4.99, 'http://entertainment.inquirer.net/files/2016/07/13717225_1265259390150735_8093269019210020606_o.jpg')
		 	brand.products.append(product)
		
		 	self.assertEqual(brand.products.count(), 1)
		 	self.assertEqual(brand.products.filter_by(name='makeup').count(), 1)
	...
	def test_model_product_brand_2 (self):
		 with app.test_request_context():
		 	...
		 	self.assertEqual(db.session.query(Product).filter_by(name='Makeup').first().Brand.name, 'Pure Anada')
